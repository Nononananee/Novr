...
class DocumentIngestionPipeline:
    ...
    def __init__(
        ...
        # Initialize components with the advanced chunker
    ...
    def _convert_novel_chunks_to_document_chunks(self, novel_chunks: List[Any], doc_title: str, doc_source: str) -> List[DocumentChunk]:
        """Converts chunks from NovelChunker to the DocumentChunk format."""
        document_chunks = []
        for i, n_chunk in enumerate(novel_chunks):
            # Adapt the structure here
            doc_chunk = DocumentChunk(
                content=n_chunk.content,
                index=i,
                start_char=0, # This info is lost in the new chunker, using placeholder
                end_char=len(n_chunk.content), # This info is lost, using placeholder
                metadata={
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        # Note: The context for adaptive_chunking is simplified here.
        novel_chunks = self.chunker.adaptive_chunking(
            text=document_content,
            context={
                'characters': [], # Character extraction now happens later
        ...
        chunks = self._convert_novel_chunks_to_document_chunks(novel_chunks, document_title, document_source)
        ...
        if self.config.extract_entities:
            # This part of the pipeline now uses the chunks' content directly
            temp_chunks_for_entity_extraction = chunks
            
            # The graph_builder expects DocumentChunk objects, which we have.
            chunks_with_entities = await self.graph_builder.extract_entities_from_chunks(temp_chunks_for_entity_extraction)
            
            # Since we're modifying metadata, we can just update the list
            chunks = chunks_with_entities
